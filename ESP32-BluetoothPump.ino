//***************sætter temp testet 16-12 virker delvis ********************************************************
// 1612 lavet kode der sætter relæer hvis temp basal er over basal - hvos temp er under basal eler på nul skal d
//er laves løsnig - uder virker ikke endnu
...
uint8_t bolusNumber; //Number of times to deliver minBolus
uint8_t bolusTime //Time between each minBolus, in minuts.
const float minBolus = 0.1; //Minimum bolus delivery possible

const float maxDeltaBasal = 3.0; //Maximum delta basal rate.

uint8_t tempRuntime; //Time the temp has been run, in min.
...
String inputString;// fra Lasse - seriel input
int value_bolus_AAPS;// fra Lasse - seriel input
int value_result_bolus;//resultet => antal gange bolus a 0,1
int i;
...
void baseBasalRate(String command) {
  ...
  if (newBaseBasal != baseBasal) {
    ...
    Serial.println("Ny baseBasal rate");
  }
}
//**************************************************************************************************************
...
void newTempBasal(String command) {
  ...
  if (tempBasal > baseBasal && tempBasal) {
    setPumpStatus(pON);
    deltaBasal = tempBasal - baseBasal;
    if (deltaBasal > maxDeltaBasal) { //Make sure that deltaBasal does not become larger then maxDeltaBasal
      deltaBasal = maxDeltaBasal;
    }
    bolusNumber = (deltaBasal / minBolus) + 0.5;
    bolusTime = bolusNumber / tempDuration;
    tempRuntime = 0;
    tempActive = true;
...
void turnPumpOff() {
  Serial.println("Stopper pumpe");
  // ---- Funktion for at give Bolus start ---------------------------------------------
  ...
  for (i = 0; i < 2; i++) {// Loop to do "something" n times
...
void manageTreatment () {
  if (tempActive) {
    if ((millis() - prevTreatmentTime) >= (bolusTime * 1000)) {

      
      Serial.println("Giver 1U");
      // ---- Funktion for at give Bolus start ---------------------------------------------
      digitalWrite(ACT, HIGH);       // sets the ACT digital pin 18 on
      delay(3000);                  // waits for 2 second
      digitalWrite(ACT, LOW);       // sets the ACT digital pin 18 OFF
      ...
      for (i = 0; i < 5 ; i++) {// Loop to do "something" n times
        ...
        delay(1000);                  // waits for 2 second            ------- dettes skal gøres i X gange for af´t give rigt mængde // Do "something"
      }
...
void setup() {
  ...
  delay(1000);
  getBaseBasalRate("getBaseBasalRate basal rate: 0.9");
  setTempBasalAbsolute("EnactPumpResult|setTempBasalAbsolute tempBasal: 0.0 duration: 120");
